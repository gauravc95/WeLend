...
contract Mortgage {
  ...
  function addData(bytes32 document) public {
    address[] memory owners = ownerMap[document];
    uint i;
  ...
  function getMortgageCount(address person) public returns(uint) {
    return mortgageMap[person].length;
  }

  function getOwnerCount(bytes32 hash) public returns(uint) {
    return ownerMap[hash].length;
  }
  
  function getOwnerByPosition(bytes32 hash,uint index) public returns(address) {
