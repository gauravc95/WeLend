...
contract CrowdBank {
    ...
    function hasActiveLoan(address borrower) public returns(bool) {
        uint validLoans = loanMap[borrower].length;
        if(validLoans == 0) return false;
         Loan memory obj = loanList[loanMap[borrower][validLoans-1]];
    ...
    function getActiveLoanId(address borrower) public returns(uint) {
        ...
        if(loanList[loanId].state == LoanState.ACCEPTING)
        {
          ...
          for(uint i = 0; i < loanList[loanId].proposalCount; i++)
          {
            ...
            else
            {
              ...
              address(uint160(proposalList[numI].lender)).transfer(proposalList[numI].amount); //Send back to lender
            }
          }
        }
        else
    ...
    //Amount to be Repaid
    function getRepayValue(uint loanId) public returns(uint) {
      ...
      if(paid >= toBePaid)
      {
        ...
        for(uint i = 0; i < loanList[loanId].proposalCount; i++)
        {
          ...
          if(proposalList[numI].state == ProposalState.ACCEPTED)
          {
            ...
            address(uint160(proposalList[numI].lender)).transfer(finalamount);
            proposalList[numI].state = ProposalState.REPAID;
          }
        }
    ...
    function acceptProposal(uint proposeId) public
    {
        ...
        Proposal memory pObj = proposalList[proposeId];
        if(pObj.state != ProposalState.WAITING) return;

        Loan  memory lObj = loanList[loanId];
    ...
    function totalProposalsBy(address lender) public returns(uint) {
        return lendMap[lender].length;
    }

    function getProposalAtPosFor(address lender, uint pos) public returns(address, uint, ProposalState, uint, uint, uint, uint, bytes32) {
        Proposal  memory prop = proposalList[lendMap[lender][pos]];
...
// BORROWER ACTIONS AVAILABLE    
    ...
    function totalLoansBy(address borrower) public returns(uint) {
        return loanMap[borrower].length;
    }

    function getLoanDetailsByAddressPosition(address borrower, uint pos) public returns(LoanState, uint, uint, uint, uint,bytes32) {
        Loan  memory obj = loanList[loanMap[borrower][pos]];
        return (obj.state, obj.dueDate, obj.amount, obj.collected, loanMap[borrower][pos], obj.mortgage);
    }

    function getLastLoanState(address borrower) public returns(LoanState) {
    ...
    function getLastLoanDetails(address borrower) public returns(LoanState, uint, uint, uint, uint) {
        uint loanLength = loanMap[borrower].length;
        Loan memory obj = loanList[loanMap[borrower][loanLength -1]];
        return (obj.state, obj.dueDate, obj.amount, obj.proposalCount, obj.collected);
    }

    function getProposalDetailsByLoanIdPosition(uint loanId, uint numI) public returns(ProposalState, uint, uint, uint, address) {
        Proposal memory obj = proposalList[loanList[loanId].proposal[numI]];
        return (obj.state, obj.rate, obj.amount, loanList[loanId].proposal[numI],obj.lender);
    }

    function numTotalLoans() public returns(uint) {
